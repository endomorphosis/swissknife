...
const MGW_565_DAEMON_LAUNCH_GATE_FIXTURE = path.join(
  process.cwd(),
  '..',
  'hallucinate_app',
  'test',
...
test('MGW-565 daemon launch gate fixture preserves Swissknife backend handoff records', async () => {
  const fixture = JSON.parse(fs.readFileSync(MGW_565_DAEMON_LAUNCH_GATE_FIXTURE, 'utf8'));

  expect(fixture.task_id).toBe('MGW-565');
  expect(fixture.goal_id).toBe('VAIOS-G728');
  expect(fixture.goal_packet).toBe('goal_packet/launch/hallucinate_app/44dceea6bc53');
  expect(fixture.packet_goals).toEqual(['VAIOS-G724', 'VAIOS-G728']);
  expect(fixture.evidence_term).toBe('launch Playwright validation gate');
  expect(fixture.supervisor_gap_receipt).toBe(
    'data/meta_glasses_display_widgets/discovery/2026-07-02-mgw-565-objective-gap-b023c8de5b69.md',
  );
  expect(fixture.objective_gap_receipt).toBe(fixture.supervisor_gap_receipt);
  expect(fixture.launch_gate_receipt).toBe(
