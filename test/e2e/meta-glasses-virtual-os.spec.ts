...
const VAI_681_DAEMON_LAUNCH_GATE_FIXTURE = path.join(
  process.cwd(),
  '..',
  'hallucinate_app',
  'test',
  'e2e',
  'fixtures',
  'vai-681-daemon-launch-health-gate.json',
);
...
test('VAI-681 daemon launch gate fixture preserves Swissknife backend handoff records', async () => {
  const fixture = JSON.parse(fs.readFileSync(VAI_681_DAEMON_LAUNCH_GATE_FIXTURE, 'utf8'));

  expect(fixture.task_id).toBe('VAI-681');
  expect(fixture.goal_id).toBe('VAIOS-G728');
  expect(fixture.goal_packet).toBe('goal_packet/launch/hallucinate_app/44dceea6bc53');
  expect(fixture.packet_goals).toEqual(['VAIOS-G724', 'VAIOS-G728']);
  expect(fixture.evidence_term).toBe('launch Playwright validation gate');
  ...
  expect(fixture.todo_source).toEqual({
