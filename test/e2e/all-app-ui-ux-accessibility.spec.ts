...
type ViewportEvidence = {
  ...
  keyboard_focus: { operation: boolean; retry: boolean; focus_order: string[] };
  ...
  backend_failure: { action_remained_visible: boolean; error_visible: boolean; recovery_visible: boolean };
  permission_failure: { action_remained_visible: boolean; error_visible: boolean; recovery_visible: boolean };
  status: 'passed';
};
  ...
  test('proves every application retains accessible, readable failure recovery controls on desktop and mobile simulators', async ({ page }) => {
    ...
    for (const scenario of scenarios) {
      ...
      for (const viewport of VIEWPORTS) {
        ...
        const stableControls = equalBounds(initialControlBounds, finalControlBounds);
        const readable = layout.fully_visible && !layout.horizontal_overflow
          && layout.no_critical_text_overlap && layout.critical_text_count >= 5;
        const viewportEvidence: ViewportEvidence = {
          viewport,
          stable_controls: stableControls,
          keyboard_focus: { operation: operationFocus, retry: retryFocus, focus_order: focusOrder },
          accessible_names: {
    ...
    const report = {
      ...
      acceptance: {
        ...
        keyboard_focus_and_accessible_names: apps.every(app => app.viewports.every(viewport => viewport.keyboard_focus.operation
          && viewport.keyboard_focus.retry && Object.values(viewport.accessible_names).every(Boolean))),
        readable_status_error_and_recovery_text: apps.every(app => app.viewports.every(viewport => viewport.readable_status_and_errors)),
        no_clipped_or_overlapping_critical_text: apps.every(app => app.viewports.every(viewport => viewport.layout.fully_visible
          && !viewport.layout.horizontal_overflow && viewport.layout.no_critical_text_overlap)),
...
async function captureLayout(page: Page): Promise<LayoutEvidence> {
  return page.getByTestId('simulator-app').evaluate(element => {
    ...
    const critical = [...element.querySelectorAll<HTMLElement>('[data-critical-text]')].map(item => {
      ...
      return { id: item.dataset.criticalText ?? 'unknown', left: bounds.left, right: bounds.right, top: bounds.top, bottom: bounds.bottom, width: bounds.width, height: bounds.height };
    });
