...
} from '../../src/services/mcp/mcp-plus-plus-a2a';
...
describe('SwissKnifeA2AAdapter@1 interface', () => {
  it('pins interface constants and confirmed wire URI', () => {
    ...
    expect(adapter.extensionUri).toBe('https://mcplusplus.io/extensions/execution/v1');
  ...
  it('creates independent agents with distinct Event DAGs', () => {
    ...
    const client = adapter.createAgent({ agentId: 'client-a', name: 'Client A' });
    const server = adapter.createAgent({ agentId: 'server-b', name: 'Server B' });
  ...
  it('presents the execution extension on the Agent Card equivalent', () => {
    ...
    const extensions = (card.capabilities as { extensions: unknown[] }).extensions;
    expect(extensions).toHaveLength(1);
    expect((extensions[0] as { uri: string }).uri).toBe(EXTENSION_URI);
    const result = validateAgentExtension(extensions[0]);
    expect(result.ok).toBe(true);
    expect((extensions[0] as { params: { alias: string } }).params.alias).toBe(WORKING_ALIAS);
    expect((card.metadata as Record<string, unknown>)[`${METADATA_KEY_PREFIX}runtime`]).toBe(
      'swissknife',
    );
  });
});
...
describe('two-agent handoff', () => {
  it('completes an evidence-bearing handoff between independent agents', () => {
    const adapter = new SwissKnifeA2AAdapter();
    const client = adapter.createAgent({ agentId: 'client-a', name: 'Client Agent A' });
    const server = adapter.createAgent({ agentId: 'server-b', name: 'Server Agent B' });
    ...
    expect(task.artifacts[0].metadata[`${METADATA_KEY_PREFIX}output_cid`]).toBeTruthy();
  });
  ...
  it('accepts A2A-Extensions as a header string', () => {
    ...
    const receipt = adapter.handoff(client, server, { a2aExtensions: EXTENSION_URI });
  ...
  it('rejects unsupported and non-subset profiles', () => {
    ...
    expect(() =>
      adapter.handoff(client, server, { requestedProfiles: ['Z'] as unknown as string[] }),
    ).toThrow(A2AExtensionError);
    try {
      adapter.handoff(client, server, { requestedProfiles: ['Z'] as unknown as string[] });
    } catch (error) {
  ...
  it('writes Event DAG cancel records', () => {
    ...
    const open = adapter.handoff(client, server, { holdOpen: true, execute: false });
    const taskId = String((open.task as { id: string }).id);
    const canceled = adapter.cancel(server, taskId, { reason: 'test-cancel' });
    expect((canceled.task as { status: { state: string } }).status.state).toBe(TaskState.CANCELED);
    expect((canceled.cancel_events as unknown[]).length).toBeGreaterThanOrEqual(1);
    expect(Object.keys(canceled.durable_cancels as object).length).toBeGreaterThanOrEqual(1);
    expect((canceled.event_lineage as string[]).length).toBeGreaterThanOrEqual(1);
  });
});
