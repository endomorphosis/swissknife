...
describe('TrafficManager', () => {
  ...
  jest.setTimeout(120000);
        ...
        it('should select a server based on traffic allocation', async () => {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                const serverNameArg = connectToServer.mock.calls[lastCall][0];
                if (serverNameArg.includes('1.0.0')) {
                    selections['1.0.0']++;
                }
                else if (serverNameArg.includes('2.0.0')) {
                    selections['2.0.0']++;
                }
            }
        ...
        it('should filter servers based on version constraint', async () => {
            ...
            for (const call of connectToServer.mock.calls) {
                const serverNameArg = call[0];
                if (serverNameArg.includes('1.5.0') || serverNameArg.includes('2.0.0')) {
                    foundMatch = true;
                    break;
                }
            }
  ...
  jest.setTimeout(120000);
        ...
        it('should gradually shift traffic between versions', async () => {
            ...
            const shiftPromise = trafficManager.shiftTraffic('test-server', '1.0.0', // from
            '2.0.0', // to
            100, // target
            ...
            expect(mockRegistry.updateTrafficPercentage).toHaveBeenCalledTimes(8);
...
//# sourceMappingURL=mcp-traffic-manager.test.js.map