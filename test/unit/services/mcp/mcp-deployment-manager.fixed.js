...
describe('DeploymentManager', () => {
    ...
    describe('deployVersion', () => {
        ...
        it('should deploy a new version as green with 0% traffic by default', async () => {
            ...
            // Deploy the version
            const result = await deploymentManager.deployVersion('test-server', '1.0.0', {
                type: 'stdio',
                command: 'echo',
                args: ['test'],
            });
            ...
            expect(mockRegistry.registerServer).toHaveBeenCalledWith('test-server', expect.objectContaining({
                type: 'stdio',
                command: 'echo',
                args: ['test'],
                version: '1.0.0',
        ...
        it('should deploy with custom options when provided', async () => {
            ...
            const result = await deploymentManager.deployVersion('test-server', '2.0.0', {
                type: 'sse',
        ...
        it('should demote existing blue version when deploying a new blue version', async () => {
            ...
            await deploymentManager.deployVersion('test-server', '2.0.0', {
                type: 'stdio',
                command: 'echo',
                args: ['test'],
            }, {
            ...
            expect(mockRegistry.updateServerStatus).toHaveBeenCalledWith('test-server', '1.0.0', 'inactive');
        });
        it('should validate version format', async () => {
            await expect(deploymentManager.deployVersion('test-server', 'invalid-version', {
                type: 'stdio',
                command: 'echo',
                args: ['test'],
        ...
        it('should reject if version already exists', async () => {
            ...
            await expect(deploymentManager.deployVersion('test-server', '1.0.0', {
                type: 'stdio',
                command: 'echo',
                args: ['test'],
    ...
    describe('promoteToBlue', () => {
        ...
        it('should promote a version to blue and give it 100% traffic', async () => {
            ...
            expect(result).toBe(true);
            // Check registry calls
            expect(mockRegistry.updateServerStatus).toHaveBeenCalledWith('test-server', '1.0.0', 'blue');
            expect(mockRegistry.updateTrafficPercentage).toHaveBeenCalledWith('test-server', '1.0.0', 100);
        });
        ...
        it('should throw if version does not exist', async () => {
            ...
            await expect(deploymentManager.promoteToBlue('test-server', '1.0.0')).rejects.toThrow('not found');
        });
    });
    describe('rollback', () => {
        beforeEach(async () => {
        ...
        it('should roll back from problematic version to blue version', async () => {
            ...
            // Perform rollback
            const result = await deploymentManager.rollback('test-server', '2.0.0', 'Test failure');
            expect(result).toBe(true);
            // Check registry calls
            expect(mockRegistry.updateServerStatus).toHaveBeenCalledWith('test-server', '2.0.0', 'inactive');
            expect(mockRegistry.updateTrafficPercentage).toHaveBeenCalledWith('test-server', '2.0.0', 0);
            expect(mockRegistry.updateTrafficPercentage).toHaveBeenCalledWith('test-server', '1.0.0', 100);
            expect(mockRegistry.recordRollback).toHaveBeenCalledWith('test-server', '2.0.0', '1.0.0', 'Test failure');
        });
        it('should throw if no blue version exists', async () => {
            ...
            await expect(deploymentManager.rollback('test-server', '2.0.0', 'Test failure')).rejects.toThrow('No blue version found');
        });
        it('should throw if trying to roll back the blue version', async () => {
            ...
            await expect(deploymentManager.rollback('test-server', '1.0.0', 'Test failure')).rejects.toThrow('Cannot roll back from blue version');
        });
    });
    describe('setTrafficPercentage', () => {
        ...
        it('should update traffic percentage', async () => {
            ...
            const result = await deploymentManager.setTrafficPercentage('test-server', '2.0.0', 50);
            expect(result).toBe(true);
            expect(mockRegistry.updateTrafficPercentage).toHaveBeenCalledWith('test-server', '2.0.0', 50);
        });
        it('should throw if version does not exist', async () => {
            mockRegistry.getServerVersion.mockReturnValue(undefined);
            await expect(deploymentManager.setTrafficPercentage('test-server', '2.0.0', 50)).rejects.toThrow('not found');
        });
    });
...
//# sourceMappingURL=mcp-deployment-manager.fixed.js.map