...
describe('ServerRegistry', () => {
    ...
    beforeEach(() => {
        ...
        getCurrentProjectConfig.mockReturnValue(mockProjectConfig);
        getGlobalConfig.mockReturnValue(mockGlobalConfig);
    });
    ...
    it('should record rollback events', async () => {
        ...
        getCurrentProjectConfig.mockReturnValue(mockProjectConfig);
        // Record a rollback
        registry.recordRollback('test-server', '2.0.0', '1.0.0', 'Health check failed');
        // Check rollback was saved
        expect(saveCurrentProjectConfig).toHaveBeenCalled();
        // Capture the config that was saved
        const savedConfig = saveCurrentProjectConfig.mock.calls[0][0];
