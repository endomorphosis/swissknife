...
describe("material normalization and version compiler", () => {
  ...
  it("validates facade compile options before defaulting", () => {
    const compiler = createComponentVersionCompiler({
      extractorVersion: EXTRACTOR,
    });
    const identity = buildStableIdentity({
    ...
    const invalidOptions = [
      ...
      { optimizerSchemaVersion: null },
      { unexpected: true },
    ];
    for (const options of invalidOptions) {
      expect(() =>
        compiler.compile(identity, baseMaterial(), options as never),
      ).toThrow(/compiler facade options/);
      expect(() =>
        compiler.identityFor(identity, baseMaterial(), options as never),
      ).toThrow(/compiler facade options/);
    }
    ...
    expect(version.optimizer_schema_version).toBe(UI_COMPONENT_VERSION_SCHEMA);
    expect(
      compiler.identityFor(identity, baseMaterial(), {
        optimizerSchemaVersion: UI_COMPONENT_VERSION_SCHEMA,
      }).domain,
    ).toBe(DOMAIN_COMPONENT_VERSION);
  });
