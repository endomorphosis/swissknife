...
import { deonticNormalizedTextFromRows } from '../../src/services/logic/bridges/deontic-norms-bridge';
...
function runPythonReference(corpusPath: string): PyResults {
  const tempDir = nodeFs.mkdtempSync(join(tmpdir(), 'deontic-bridge-normalized-py-'));
