/**
 * Integration Tests for MCP Client Service (`src/services/mcp/host/mcpClient.ts`)
 *
...
import { spawn } from 'child_process'; // Import directly for type usage in mock
...
const mockSdkClientInstance = {
    ...
    disconnect: jest.fn().mockResolvedValue(undefined), // Added disconnect
    getServerCapabilities: jest.fn().mockResolvedValue({ tools: true, prompts: true }), // Example capabilities
    request: jest.fn(async (req: { method: string }, schema: any) => { // Added types
    ...
    callTool: jest.fn().mockImplementation(async (params: { name: string; arguments: any }) => ({ // Added types
        content: [{ type: 'text', text: `Result for ${params.name}` }],
    })),
    getPrompt: jest.fn().mockImplementation(async (params: { name: string; arguments: any }) => ({ // Added types
...
jest.mock('../../../src/utils/config', () => ({
    ...
    saveCurrentProjectConfig: jest.fn((config) => { // Update local mock state
...
describe('MCP Client Service Integration', () => {
  ...
  const testServerConfig = {
    type: 'stdio' as const, // Use 'as const' for literal type
    command: 'node',
  ...
  describe('addMcpServer / removeMcpServer', () => {
    ...
    it('should remove a server configuration from the project config', async () => {
      ...
      (saveCurrentProjectConfig as jest.Mock).mockClear(); // Clear previous calls
     ...
     it('should handle removing a non-existent server gracefully', async () => {
      ...
      (saveCurrentProjectConfig as jest.Mock).mockClear();
  ...
  describe('getClients / getMCPTools', () => {
    ...
    it('should connect to configured MCP servers and return client instances', async () => {
      ...
      expect((clients[0] as any)._client).toBeDefined();
    });
    ...
    it('should retrieve tools from connected MCP clients', async () => {
      ...
      // Act
      const tools: Tool[] = await getMCPTools(); // This calls client.request('tools/list')
