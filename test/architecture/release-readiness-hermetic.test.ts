...
function createWritingProducerStub(evidenceRoot: string, calls: string[]) {
  return (producer: (typeof RELEASE_EVIDENCE_PRODUCER_GATES)[number]) => {
    ...
    for (const file of producer.evidenceFiles ?? []) {
      mkdirSync(dirname(join(evidenceRoot, file)), { recursive: true });
      writeFileSync(
...
describe('release readiness hermetic cold-start orchestration (SVD-131)', () => {
  it('declares one producer for every SVD-131 evidence class in the required order', () => {
    ...
    expect(ids).toEqual([
      ...
      'all-app-improvement-release-evidence',
    ...
    const behaviorProducer = RELEASE_EVIDENCE_PRODUCER_GATES.find((producer) => producer.id === 'application-live-behavior-proof');
    expect(behaviorProducer?.dependsOn).toEqual(['release-baseline-catalogs']);
    expect(ids.indexOf('release-baseline-catalogs')).toBeLessThan(ids.indexOf('application-live-behavior-proof'));
    const allAppProducer = RELEASE_EVIDENCE_PRODUCER_GATES.find((producer) => producer.id === 'all-app-improvement-release-evidence');
    expect(allAppProducer?.dependsOn).toEqual(['release-baseline-catalogs', 'application-gateway-evidence', 'meta-device-simulator-replay']);
    expect(ids.indexOf('meta-device-simulator-replay')).toBeLessThan(ids.indexOf('all-app-improvement-release-evidence'));
  });
  ...
  it('stops at the first failing producer and never runs downstream producers out of order', () => {
    ...
    try {
      ...
      expect(calls).toEqual(['release-baseline-catalogs', 'application-live-behavior-proof', 'application-gateway-evidence']);
