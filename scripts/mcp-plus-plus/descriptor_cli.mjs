...
function main(argv) {
  ...
  try {
    switch (command) {
      case 'lint':
        return printResult(lintDescriptor(readJson(requiredArg(args, 0, 'descriptor'))));
      case 'validate':
...
function datasetInferenceWorkflowStarter(options) {
  ...
  descriptor.workflow_graph = {
    ...
    steps: [
      ...
      { id: 'publish_artifact', operation: 'publish_artifact', depends_on: ['run_inference_job'], read_state_keys: ['job_id', 'artifact_cid'], write_state_keys: ['publication_id'] },
    ],
  };
...
function help() {
  console.log(`Usage:
  descriptor_cli.mjs lint <descriptor.json>
