...
const vai570LaunchGateReceiptPath = path.resolve(
  '..',
  'hallucinate_app',
  'test',
  'e2e',
  'fixtures',
  'vai-570-mcp-dashboard-launch-gate.json',
);
...
const vai570LaunchGateReceipt = JSON.parse(fs.readFileSync(vai570LaunchGateReceiptPath, 'utf8'));
...
const vai570Gate = (catalog.launch_validation_gates || []).find(gate => gate.task_id === 'VAI-570');
assert(vai570Gate?.goal_id === 'VAIOS-G724', 'Catalog launch validation gates must include VAI-570 for VAIOS-G724');
assert(
  vai570Gate?.source_gap_receipt === 'data/virtual_ai_os/discovery/2026-07-04-vai-570-objective-gap-3e00ad2a0074.md',
  'VAI-570 launch gate must point at the current source gap receipt',
);
assert(
  vai570Gate?.launch_gate_receipt === 'data/virtual_ai_os/discovery/2026-07-04-vai-570-mcp-dashboard-launch-gate.md',
  'VAI-570 launch gate must point at the current launch gate receipt',
);
assert(
  vai570Gate?.receipt_fixture === 'hallucinate_app/test/e2e/fixtures/vai-570-mcp-dashboard-launch-gate.json',
  'VAI-570 launch gate must point at the Playwright fixture',
);
assert(
  vai570Gate?.gate_state === 'gate_closed_by_playwright_validation',
  'VAI-570 launch gate must be closed by the Hallucinate App Playwright validation gate',
);
assert(
  JSON.stringify(vai570Gate?.packet_goal_ids || []) === JSON.stringify(['VAIOS-G724', 'VAIOS-G728']),
  'VAI-570 launch gate must preserve VAIOS-G724/VAIOS-G728 packet goals',
);
assert(
  JSON.stringify(vai570Gate?.validation_commands || []) === JSON.stringify(vai570LaunchGateReceipt.validation_commands),
  'VAI-570 launch gate must preserve the shared packet validation commands',
);
assert(
  JSON.stringify(vai570Gate?.dashboard_servers || []) === JSON.stringify(vai570LaunchGateReceipt.dashboard_servers),
  'VAI-570 launch gate must preserve all dashboard server handoff records',
);
...
console.log(JSON.stringify({
  ...
  virtual_ai_launch_task_ids: [
    ...
    vai570LaunchGateReceipt.task_id,
  ],
