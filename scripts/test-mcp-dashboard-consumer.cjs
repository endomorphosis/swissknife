...
assert(
  hao729Gate?.attempt === 1,
  'HAO-729 launch gate must expose the attempt-1 validation receipt for Swissknife consumers',
);
assert(
  JSON.stringify(hao729Gate?.attempt_receipts || []) === JSON.stringify([
    'data/hallucinate_multimodal_control/discovery/2026-07-08-hao-729-attempt-1-validation.md',
  ]),
  'HAO-729 launch gate must point at the attempt-1 validation receipt',
);
assert(
  (hao729Gate?.validation_commands || []).some(command => command.includes('HALLUCINATE_APP_E2E_NO_BOOTSTRAP=true node scripts/run_playwright_test.mjs --help')),
  'HAO-729 launch gate must include the headless-safe Playwright preflight',
);
