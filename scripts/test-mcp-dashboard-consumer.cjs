...
assert(vai640Gate?.attempt === 1, 'VAI-640 launch gate must preserve the attempt-1 validation receipt number');
assert(
  JSON.stringify(vai640Gate?.attempt_receipts || []) === JSON.stringify([
    'data/virtual_ai_os/discovery/2026-07-04-vai-640-attempt-1-launch-playwright-validation-gate.md',
  ]),
  'VAI-640 launch gate must expose the attempt-1 launch Playwright validation receipt',
);
assert(
