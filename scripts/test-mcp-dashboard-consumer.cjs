...
assert(
  ...
  'HAO-727 launch gate must point at the Hallucinate supervisor gap receipt',
);
assert(
  ...
  'HAO-727 launch gate must point at the Hallucinate supervisor launch receipt',
);
assert(
  hao727Gate?.receipt_fixture === 'hallucinate_app/test/e2e/fixtures/hao-727-mcp-dashboard-launch-gate.json',
  'HAO-727 launch gate must expose the Hallucinate supervisor launch fixture',
);
assert(
...
assert(
  JSON.stringify(hao727Gate?.follow_up_subtasks || []) === JSON.stringify(['HAO-678', 'HAO-679', 'HAO-680', 'HAO-681', 'HAO-682', 'HAO-683']),
