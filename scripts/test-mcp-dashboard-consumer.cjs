...
const vai680LaunchGateReceiptPath = path.resolve(
  '..',
  'hallucinate_app',
  'test',
  'e2e',
  'fixtures',
  'vai-680-mcp-dashboard-launch-gate.json',
);
...
const vai680LaunchGateReceipt = JSON.parse(fs.readFileSync(vai680LaunchGateReceiptPath, 'utf8'));
...
const vai680Gate = (catalog.launch_validation_gates || []).find(gate => gate.task_id === 'VAI-680');
assert(vai680Gate?.goal_id === 'VAIOS-G724', 'Catalog launch validation gates must include VAI-680 for VAIOS-G724');
assert(
  vai680Gate?.supervisor_gap_receipt === 'data/virtual_ai_os/discovery/2026-07-08-vai-680-objective-gap-3e00ad2a0074.md',
  'VAI-680 launch gate must point at the current supervisor gap receipt',
);
assert(
  vai680Gate?.launch_gate_receipt === 'data/virtual_ai_os/discovery/2026-07-08-vai-680-mcp-dashboard-launch-gate.md',
  'VAI-680 launch gate must point at the current launch gate receipt',
);
assert(
  vai680Gate?.receipt_fixture === 'hallucinate_app/test/e2e/fixtures/vai-680-mcp-dashboard-launch-gate.json',
  'VAI-680 launch gate must point at the Playwright fixture',
);
assert(
  JSON.stringify(vai680Gate?.todo_source || {}) === JSON.stringify({
    file: 'implementation_plan/docs/19-virtual-ai-os-submodule-integration.todo.md',
    source_line: 9360,
  }),
  'VAI-680 launch gate must preserve the supervisor todo source line for Swissknife consumers',
);
assert(
  vai680Gate?.gate_state === 'gate_closed_by_playwright_validation',
  'VAI-680 launch gate must be closed by the launch Playwright validation gate',
);
assert(
  vai680Gate?.packet_sibling_task_id === 'VAI-681',
  'VAI-680 launch gate must preserve the VAI-681 packet sibling task',
);
assert(
  vai680Gate?.packet_sibling_gate_receipt === 'data/virtual_ai_os/discovery/2026-07-08-vai-681-daemon-launch-health-gate.md',
  'VAI-680 launch gate must point at the packet sibling daemon launch gate receipt',
);
assert(vai680Gate?.attempt === 1, 'VAI-680 launch gate must preserve the attempt-1 validation receipt number');
assert(
  JSON.stringify(vai680Gate?.attempt_receipts || []) === JSON.stringify([
    'data/virtual_ai_os/discovery/2026-07-08-vai-680-attempt-1-launch-playwright-validation-gate.md',
  ]),
  'VAI-680 launch gate must expose the attempt-1 launch Playwright validation receipt',
);
assert(
  JSON.stringify(vai680Gate?.required_evidence || []) === JSON.stringify(vai680LaunchGateReceipt.required_evidence),
  'VAI-680 launch gate must preserve dashboard capability catalog evidence terms for Swissknife consumers',
);
assert(
  JSON.stringify(vai680Gate?.external_backend_surfaces || []) === JSON.stringify([
    'external/ipfs_accelerate',
    'external/ipfs_datasets',
    'external/ipfs_kit',
  ]),
  'VAI-680 launch gate must expose the external IPFS backend surfaces for Swissknife consumers',
);
...
console.log(JSON.stringify({
  ...
  virtual_ai_launch_task_ids: [
    ...
    vai680LaunchGateReceipt.task_id,
