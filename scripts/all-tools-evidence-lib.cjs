...
const VIRTUAL_DESKTOP_APPS = [
  ...
  { id: 'p2p-chat', title: 'P2P Chat - Classic', category: 'mcp-ipfs', component: 'web/js/apps/p2p-chat.js', service_families: ['ipfs_kit_py'], capabilities: ['meta-glasses-display-webapp', 'meta-glasses-microphone', 'meta-glasses-speaker', 'orb-idl-handoff'], binding_state: 'tool_backed', rationale: 'Classic P2P chat maps to pubsub and peer-discovery IPFS/libp2p tools.' },
...
function chooseAppForPolicy(policy) {
  ...
  if (policy.service_id === 'ipfs_accelerate_py') {
    ...
    if (/(model|search|recommend)/.test(haystack)) return 'model-browser';
    if (/(run_inference|inference|execute_with_payload|submit)/.test(haystack)) return 'ai-chat';
    if (/(tools_dispatch|tools_get_schema|mcp_manifest)/.test(haystack)) return 'navi';
  ...
  if (policy.service_id === 'ipfs_datasets_py') {
    ...
    if (/(dataset|load|save|process|workflow|background_task)/.test(haystack)) return 'training-manager';
