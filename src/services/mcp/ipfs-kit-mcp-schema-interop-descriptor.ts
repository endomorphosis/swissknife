 ...
 *
 * VAI-664 objective validation repair: interface contract
 * swissknife external/ipfs_kit,
 * goal_packet/interoperability/swissknife/06921590135c,
 * tests/integration/test_swissknife_external_ipfs_kit_interop.py.
 * Earlier MGW-572 evidence uses the same objective fingerprint; VAI-664 is
 * the virtual-ai-os supervisor repair tracked in data/virtual_ai_os/discovery.
 *
 * `external/ipfs_kit` ships three copies of a `fix_mcp_schema.py` helper
 * that normalizes an (invalid) `mcpServers` JSON array back into an object
 * keyed by server name, a `deprecations_report.schema.json` contract, the
 * bucket-VFS MCP tool docs, and the DAG-PB protobuf wire format SwissKnife
 * needs to interoperate with the MerkleDAG surface. This module describes
 * that combined surface as a canonical MCP-IDL Profile A descriptor that
...
export const IPFS_KIT_MCP_SCHEMA_DESCRIPTOR_PATHS = {
  ...
  fix_mcp_schema_backup: 'external/ipfs_kit/backup/archive_clutter/fix_scripts/fix_mcp_schema.py',
  fix_mcp_schema_patches: 'external/ipfs_kit/backup/patches/fixes/fix_mcp_schema.py',
  ...
  methods: [
    {
      ...
      error_schema_cids: ['bafy_kit_err_schema_invalid'],
    },
    {
      ...
      error_schema_cids: ['bafy_kit_err_report_invalid'],
    },
    {
      name: 'ipfs_kit.bucket_vfs.export_car',
      input_schema_cid: 'bafy_kit_bucket_export_car_in',
      output_schema_cid: 'bafy_kit_bucket_export_car_out',
      error_schema_cids: ['bafy_kit_err_bucket_missing'],
    },
    {
      name: 'ipfs_kit.bucket_vfs.cross_query',
      input_schema_cid: 'bafy_kit_bucket_cross_query_in',
      output_schema_cid: 'bafy_kit_bucket_cross_query_out',
      error_schema_cids: ['bafy_kit_err_query_invalid'],
      interaction_pattern: 'request-response',
    },
    {
      name: 'ipfs_kit.dag_pb.encode_node',
      input_schema_cid: 'bafy_kit_dag_pb_encode_in',
      output_schema_cid: 'bafy_kit_dag_pb_encode_out',
      error_schema_cids: ['bafy_kit_err_dag_pb_invalid'],
    },
    {
      name: 'ipfs_kit.dag_pb.decode_node',
      input_schema_cid: 'bafy_kit_dag_pb_decode_in',
      output_schema_cid: 'bafy_kit_dag_pb_decode_out',
      error_schema_cids: ['bafy_kit_err_dag_pb_invalid'],
    },
  ],
  errors: [
...
export function buildSwissKnifeIPFSKitMCPPlusPlusCompatibilityReceipt() {
  return {
    ...
    transport: {
      kind: 'local' as const,
      endpoint: 'swissknife://ipfs-kit/mcp-schema',
      protocol_path: 'swissknife/mcp++/ipfs-kit/mcp-schema',
      auth_present: true,
