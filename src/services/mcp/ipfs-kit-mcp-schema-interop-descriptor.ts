 ...
 *
 * MGW-572 and VAI-664 repair the VAIOS-G703 objective validation gap that
 * requires `swissknife` to interoperate with `external/ipfs_kit` through
 * importable contracts, interface descriptors, runtime handoff behavior, and
 * integration tests: interface contract swissknife external/ipfs_kit,
 ...
 * `external/ipfs_kit/backup/patches/fixes/fix_mcp_schema.py`), a JSON Schema
 * for its deprecations report
 * (`external/ipfs_kit/data/deprecations_report.schema.json`), a Bucket VFS
 ...
 * describing the `PBLink`/`PBNode` MerkleDAG wire format. This module
 ...
 * pre-built `IPFS_KIT_INTERFACE`, and it provides representative
 * policy-mediated control-surface, interaction-envelope, and
 * compatibility-receipt payloads for validation. It mirrors
...
export const IPFS_KIT_MCP_SCHEMA_DESCRIPTOR_PATHS = {
  fix_mcp_schema_archive_archive_clutter:
    'external/ipfs_kit/archive/archive_clutter/fix_scripts/fix_mcp_schema.py',
  fix_mcp_schema_backup_archive_clutter:
    'external/ipfs_kit/backup/archive_clutter/fix_scripts/fix_mcp_schema.py',
  fix_mcp_schema_backup_patches_fixes: 'external/ipfs_kit/backup/patches/fixes/fix_mcp_schema.py',
...
export const SWISSKNIFE_IPFS_KIT_INTEROP_INTERFACE: MCPPPInterfaceDescriptor = {
  ...
  methods: [
    ...
    },
    {
      ...
      error_schema_cids: ['bafy_ipfs_kit_err_invalid_report'],
    },
    {
    ...
    },
    {
      ...
      error_schema_cids: ['bafy_ipfs_kit_err_dag_pb_encode'],
    },
    {
      ...
      error_schema_cids: ['bafy_ipfs_kit_err_dag_pb_decode'],
    },
  ],
  errors: [
    ...
    { name: 'DagPbEncodeError', code: 500 },
    { name: 'DagPbDecodeError', code: 500 },
  ],
...
export function buildSwissKnifeIPFSKitMCPPlusPlusCompatibilityReceipt() {
  return {
    ...
    transport: {
      ...
      protocol_path: 'swissknife/mcp++/ipfs-kit/bucket-vfs',
      auth_present: true,
