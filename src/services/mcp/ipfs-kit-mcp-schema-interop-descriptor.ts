 ...
 *
 * `external/ipfs_kit` ships three copies of an MCP-settings-schema repair
 * script (`fix_mcp_schema()`, which normalizes an (invalid) `mcpServers`
 * JSON array back into an object keyed by server name), a JSON Schema for
 * its API-deprecations report, a Bucket VFS CLI/MCP interface summary
...
export const SWISSKNIFE_IPFS_KIT_INTEROP_INTERFACE: MCPPPInterfaceDescriptor = {
  ...
  methods: [
    ...
    },
    {
      ...
      error_schema_cids: ['bafy_ipfs_kit_err_invalid_node'],
    },
    {
  ...
  errors: [
    { name: 'InvalidSchema', code: 400 },
    { name: 'InvalidReport', code: 400 },
