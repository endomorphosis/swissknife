 ...
 *
 * MGW-572 objective validation repair: interface contract swissknife external/ipfs_kit,
 ...
 * `backup/patches/fixes/fix_mcp_schema.py`) that normalize a `mcpServers`
 * settings block from an (invalid) JSON array back into an object keyed by
 * server name, a JSON Schema for its deprecations report
 * (`data/deprecations_report.schema.json`), a Bucket VFS CLI/MCP interface
 * summary (`docs/implementation/BUCKET_VFS_INTERFACES_COMPLETE.md`)
 * documenting its `bucket_*` MCP tool surface, and a DAG-PB protobuf schema
 ...
 * interoperate with that surface as a canonical MCP-IDL (Profile A)
 * interface descriptor registered on the same `MCPPlusPlus` runtime
 * registry as the pre-built `IPFS_KIT_INTERFACE`, and that SwissKnife's
 * policy-mediation control surface contract and interaction envelope
 * schemas can carry `external/ipfs_kit` MCP-schema/bucket-VFS surface
 * events end to end.
 *
...
// --- Bucket VFS MCP tools + DAG-PB messages the interop contract requires ---
...
export const SWISSKNIFE_IPFS_KIT_INTEROP_INTERFACE: MCPPPInterfaceDescriptor = {
  ...
  methods: [
    ...
    },
    {
      ...
      input_schema_cid: 'bafy_exportcar_in',
      output_schema_cid: 'bafy_exportcar_out',
      error_schema_cids: ['bafy_err_notfound'],
    },
    {
      name: 'ipfs_kit.bucket_vfs.cross_query',
      input_schema_cid: 'bafy_crossquery_in',
      output_schema_cid: 'bafy_crossquery_out',
      error_schema_cids: ['bafy_err_io'],
    },
    {
      name: 'ipfs_kit.dag_pb.encode_node',
      input_schema_cid: 'bafy_encodenode_in',
      output_schema_cid: 'bafy_encodenode_out',
      error_schema_cids: ['bafy_err_io'],
    },
    {
      name: 'ipfs_kit.dag_pb.decode_node',
      input_schema_cid: 'bafy_decodenode_in',
      output_schema_cid: 'bafy_decodenode_out',
      error_schema_cids: ['bafy_err_notfound'],
    },
  ],
  errors: [
    { name: 'SchemaInvalid', code: 400 },
...
export const SWISSKNIFE_IPFS_KIT_INTEROP_DESCRIPTOR = {
  ...
  schema_refs: {
    ...
    fix_mcp_schema_backup_archive: 'external/ipfs_kit/backup/archive_clutter/fix_scripts/fix_mcp_schema.py',
  ...
  validation: {
    task_id: 'MGW-572',
    goal_id: 'VAIOS-G703',
    objective_gap_ref: 'data/meta_glasses_display_widgets/discovery/2026-07-08-mgw-572-objective-gap-f463532ba4e3.md',
    validation_repair_ref: 'data/meta_glasses_display_widgets/discovery/2026-07-08-mgw-572-objective-validation-repair.md',
    evidence: 'objective validation repair',
  },
};

/**
 * Registers the SwissKnife<->external/ipfs_kit MCP-schema/bucket-VFS
 ...
 * MCP-schema/bucket-VFS interop descriptor, so callers get a fully wired
 ...
 * SwissKnife<->external/ipfs_kit MCP-schema/bucket-VFS handoff.
 */
