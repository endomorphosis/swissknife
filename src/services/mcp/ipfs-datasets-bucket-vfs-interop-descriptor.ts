 ...
 *
 * VAI-663 objective validation repair: interface contract swissknife external/ipfs_datasets,
 * goal_packet/interoperability/swissknife/06921590135c,
 * tests/integration/test_swissknife_external_ipfs_datasets_interop.py.
 *
 * `external/ipfs_datasets` ships a nested `ipfs_kit_py` tool surface under
...
// --- Bucket VFS and unified bucket surfaces the interop contract requires ---
...
// --- Profile A: MCP-IDL interface descriptor for the SwissKnife<->external/ipfs_datasets surface ---
...
export const SWISSKNIFE_IPFS_DATASETS_INTEROP_INTERFACE: MCPPPInterfaceDescriptor = {
  ...
  version: '0.1.0',
  interface_cid: 'bafyswissknifeipfsdatasetsbucketvfs000001',
  methods: [
    {
      name: 'ipfs_datasets.bucket_vfs.create_bucket',
      input_schema_cid: 'bafy_datasets_bucketcreate_in',
      output_schema_cid: 'bafy_datasets_bucketcreate_out',
      error_schema_cids: ['bafy_err_schema_invalid', 'bafy_err_io'],
    },
    {
      name: 'ipfs_datasets.bucket_vfs.add_file',
      input_schema_cid: 'bafy_datasets_bucketaddfile_in',
      output_schema_cid: 'bafy_datasets_bucketaddfile_out',
      error_schema_cids: ['bafy_err_notfound', 'bafy_err_io'],
    },
    {
      name: 'ipfs_datasets.bucket_vfs.export_car',
      input_schema_cid: 'bafy_datasets_bucketexportcar_in',
      output_schema_cid: 'bafy_datasets_bucketexportcar_out',
      error_schema_cids: ['bafy_err_notfound', 'bafy_err_io'],
    },
    {
      name: 'ipfs_datasets.bucket_vfs.cross_query',
      input_schema_cid: 'bafy_datasets_bucketcrossquery_in',
      output_schema_cid: 'bafy_datasets_bucketcrossquery_out',
      error_schema_cids: ['bafy_err_notfound'],
    },
    {
      name: 'ipfs_datasets.unified_bucket.create_backend_bucket',
      input_schema_cid: 'bafy_datasets_unifiedcreate_in',
      output_schema_cid: 'bafy_datasets_unifiedcreate_out',
      error_schema_cids: ['bafy_err_backend_unavailable'],
    },
    {
      name: 'ipfs_datasets.unified_bucket.sync_indices',
      input_schema_cid: 'bafy_datasets_syncindices_in',
      output_schema_cid: 'bafy_datasets_syncindices_out',
      error_schema_cids: ['bafy_err_io'],
    },
    {
      name: 'ipfs_datasets.deprecations.validate_report',
      input_schema_cid: 'bafy_datasets_deprecations_in',
      output_schema_cid: 'bafy_datasets_deprecations_out',
...
/**
 * Registers the SwissKnife<->external/ipfs_datasets Bucket VFS interop
 * interface descriptor with a live `MCPPlusPlus` client instance. The
 * descriptor participates in the same runtime registry
 ...
 * (`createMCPPlusPlusClient`) plus the SwissKnife<->external/ipfs_datasets
 * Bucket VFS interop descriptor.
 */
 ...
 * SwissKnife<->external/ipfs_datasets Bucket VFS handoff.
 */
...
export function buildSwissKnifeIPFSDatasetsInteractionEnvelope() {
  return {
    ...
    logic_bindings: [
      {
        binding_id: IPFS_DATASETS_LOGIC_BINDING.binding_id,
        policy_bundle_ref: IPFS_DATASETS_POLICY_BUNDLE_REF,
        compiled_policy_cid: 'local:swissknife:ipfs-datasets-bucket-vfs-interop',
