...
type PolicyEventListener = (...args: any[]) => void;

class LocalEventEmitter {
  private readonly listeners = new Map<string, Set<PolicyEventListener>>();

  on(event: string, listener: PolicyEventListener): this {
    const bucket = this.listeners.get(event) ?? new Set<PolicyEventListener>();
    bucket.add(listener);
    this.listeners.set(event, bucket);
    return this;
  }

  off(event: string, listener: PolicyEventListener): this {
...
export class PolicyEngine extends LocalEventEmitter {
