 ...
 *
 * `Mcp-Plus-Plus` is the shared MCP-IDL/UCAN/deontic-policy/event-DAG/P2P
 * protocol suite SwissKnife builds its own `mcp-plus-plus.ts` runtime on top
 * of. This module publishes SwissKnife's own MCP++ client surface as a
 * canonical MCP-IDL Profile A descriptor (validated by the upstream
 * `Mcp-Plus-Plus` `MCPIDLValidator` via `toMcpIdlValidatorDescriptor`), and
 * it provides representative policy-mediated control-surface and
...
/**
 * Convert the SwissKnife MCP++ interface descriptor into the shape the
 * upstream `Mcp-Plus-Plus` `MCPIDLValidator` (tests-py/validators/mcp_idl.py)
 * expects: the required fields (`name`, `namespace`, `version`, `methods`,
 * `errors`, `requires`, `compatibility`) plus the recommended
 * `semantic_tags`/`observability` fields, without the internal
 * `interface_cid` bookkeeping field the validator recomputes itself.
 */
export function toMcpIdlValidatorDescriptor(
  ...
  const { interface_cid: _interface_cid, ...rest } = descriptor;
  return {
    ...rest,
    methods: descriptor.methods.map(({ name, input_schema_cid, output_schema_cid }) => ({
      name,
      input_schema_cid,
      output_schema_cid,
    })),
