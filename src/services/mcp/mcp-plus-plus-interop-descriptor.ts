 ...
 *
 * `Mcp-Plus-Plus` is the reference implementation and conformance suite for
 * the MCP-IDL / MCP++ protocol (Profile A interface discovery, Profile B
 * CID-addressed execution envelopes, Profile C UCAN capability delegation,
 * Profile D deontic policy evaluation, and Profile E P2P transport). This
 * module describes SwissKnife's own MCP++ runtime (`./mcp-plus-plus.ts`) as
 * an MCP-IDL Profile A descriptor that the shared
 * `Mcp-Plus-Plus/tests-py/validators/mcp_idl.py` validator can accept, and it
...
export const MCP_PLUS_PLUS_CONFORMANCE_FIXTURE_PATHS = {
  mcp_idl_descriptor_fixture: 'Mcp-Plus-Plus/tests-py/fixtures/valid/mcp_idl_descriptor.json',
  swissknife_interop_descriptor_fixture:
  ...
  methods: [
    {
      ...
      input_schema_cid: 'bafy_mcppp_negotiate_capabilities_in',
      output_schema_cid: 'bafy_mcppp_negotiate_capabilities_out',
      error_schema_cids: ['bafy_mcppp_err_no_common_profile'],
    },
    {
      name: 'mcpplusplus.execute_with_envelope',
      input_schema_cid: 'bafy_mcppp_execute_with_envelope_in',
      output_schema_cid: 'bafy_mcppp_execute_with_envelope_out',
      error_schema_cids: ['bafy_mcppp_err_envelope_invalid'],
    },
    {
      name: 'mcpplusplus.create_delegation',
      input_schema_cid: 'bafy_mcppp_create_delegation_in',
      output_schema_cid: 'bafy_mcppp_create_delegation_out',
      error_schema_cids: ['bafy_mcppp_err_delegation_denied'],
    },
    {
      name: 'mcpplusplus.evaluate_policy',
      input_schema_cid: 'bafy_mcppp_evaluate_policy_in',
      output_schema_cid: 'bafy_mcppp_evaluate_policy_out',
      error_schema_cids: ['bafy_mcppp_err_policy_evaluation_failed'],
    },
    {
      name: 'mcpplusplus.get_dag_frontier',
      input_schema_cid: 'bafy_mcppp_get_dag_frontier_in',
      output_schema_cid: 'bafy_mcppp_get_dag_frontier_out',
      error_schema_cids: [],
    },
    {
      name: 'mcpplusplus.check_compatibility',
      input_schema_cid: 'bafy_mcppp_check_compatibility_in',
      output_schema_cid: 'bafy_mcppp_check_compatibility_out',
      error_schema_cids: ['bafy_mcppp_err_incompatible'],
    },
    {
      name: 'mcpplusplus.create_p2p_session',
      input_schema_cid: 'bafy_mcppp_create_p2p_session_in',
      output_schema_cid: 'bafy_mcppp_create_p2p_session_out',
      error_schema_cids: ['bafy_mcppp_err_p2p_session_failed'],
      interaction_pattern: 'request-response',
    },
  ],
  errors: [
    { name: 'NoCommonProfile', code: 409 },
    { name: 'EnvelopeInvalid', code: 422 },
    { name: 'DelegationDenied', code: 403 },
    { name: 'PolicyEvaluationFailed', code: 500 },
    { name: 'Incompatible', code: 409 },
    { name: 'P2PSessionFailed', code: 502 },
  ],
...
export const SWISSKNIFE_MCP_PLUS_PLUS_INTEROP_DESCRIPTOR = {
  ...
  schema_refs: {
    ...
    ...MCP_PLUS_PLUS_CONFORMANCE_FIXTURE_PATHS,
  },
  runtime_handoff: {
    source_surface: 'swissknife',
    target_surface: 'Mcp-Plus-Plus',
...
/**
 * Projects the SwissKnife MCP++ interface descriptor into the shape the
 * shared `Mcp-Plus-Plus/tests-py/validators/mcp_idl.py` `MCPIDLValidator`
 * expects for an `interface_descriptor` message.
 */
export function toMcpIdlValidatorDescriptor(
  ...
  return {
    ...
    methods: descriptor.methods.map((method) => ({
      ...
      error_schema_cids: method.error_schema_cids,
      interaction_pattern: method.interaction_pattern ?? 'request-response',
    })),
...
export function buildSwissKnifeMcpPlusPlusCompatibilityReceipt() {
  return {
    ...
    transport: {
      kind: 'local' as const,
      endpoint: 'swissknife://mcp-plus-plus',
      protocol_path: 'swissknife/mcp++/mcp_plus_plus',
      auth_present: true,
      redaction_profile: 'mcp-plus-plus-session-minimal',
    },
    tool_call: {
      tool_name: 'mcpplusplus.execute_with_envelope',
      tool_category: 'protocol',
