 ...
 *
 * VAI-665 repairs the VAIOS-G704 objective validation gap that requires
 * `swissknife` to interoperate with `Mcp-Plus-Plus` through importable
 * contracts, interface descriptors, runtime handoff behavior, and
 * integration tests: interface contract swissknife Mcp-Plus-Plus,
 * goal_packet/interoperability/swissknife/06921590135c,
 * tests/integration/test_swissknife_mcp_plus_plus_interop.py.
 *
 * `Mcp-Plus-Plus` (the upstream MCP-IDL/MCP++ protocol reference
 * implementation) ships a Python `MCPIDLValidator`
 * (`Mcp-Plus-Plus/tests-py/validators/mcp_idl.py`) that can accept any
 * interface descriptor conforming to its MCP-IDL Profile A schema. This
 * module describes SwissKnife's own MCP++ negotiation/execution/policy/DAG/
 * P2P surface as a canonical `MCPPPInterfaceDescriptor`
 * (`SWISSKNIFE_MCP_PLUS_PLUS_INTEROP_INTERFACE`) that can be registered on
 * a live `MCPPlusPlus` runtime registry, and provides
 * `toMcpIdlValidatorDescriptor()` to convert that descriptor into the exact
 * JSON shape the upstream `MCPIDLValidator` expects (mirrored on disk at
 * `Mcp-Plus-Plus/tests-py/fixtures/valid/swissknife_mcp_plus_plus_interop_descriptor.json`),
 * plus representative policy-mediated control-surface and
...
export const MCP_PLUS_PLUS_FIXTURE_PATHS = {
  ...
  methods: [
    {
      ...
      error_schema_cids: ['bafy_mpp_err_unsupported_profile'],
    },
    {
      name: 'mcpplusplus.execute_with_envelope',
      input_schema_cid: 'bafy_execute_in',
      output_schema_cid: 'bafy_execute_out',
      error_schema_cids: ['bafy_mpp_err_policy_mediation_required'],
    ...
    },
    {
      ...
      error_schema_cids: ['bafy_mpp_err_compatibility_check_failed'],
    },
    {
...
/**
 * Convert `SWISSKNIFE_MCP_PLUS_PLUS_INTEROP_INTERFACE` into the plain JSON
 * shape the upstream Mcp-Plus-Plus Python `MCPIDLValidator` accepts (see
 * `Mcp-Plus-Plus/tests-py/fixtures/valid/swissknife_mcp_plus_plus_interop_descriptor.json`).
 */
