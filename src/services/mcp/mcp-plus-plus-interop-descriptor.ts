 ...
 *
 * `Mcp-Plus-Plus` is the upstream MCP++ protocol reference implementation
 * (Profile A MCP-IDL, Profile B CID-native execution, Profile C UCAN
 * delegation, Profile D deontic policy, Profile E p2p transport, and the
 * append-only event DAG). This module describes SwissKnife's own MCP-IDL
 * Profile A descriptor for that surface, mirrors it into a validator-ready
 * fixture consumable by the upstream `Mcp-Plus-Plus` MCPIDLValidator via
 * `toMcpIdlValidatorDescriptor`, and provides representative
  ...
  methods: [
    {
      ...
      error_schema_cids: ['bafy_mcpplusplus_err_unsupported_profile'],
    },
    {
      name: 'mcpplusplus.execute_with_envelope',
      input_schema_cid: 'bafy_execute_in',
      output_schema_cid: 'bafy_execute_out',
      error_schema_cids: ['bafy_mcpplusplus_err_policy_mediation_required'],
    ...
    },
    {
      ...
      error_schema_cids: ['bafy_mcpplusplus_err_compatibility_check_failed'],
    },
    {
...
export const SWISSKNIFE_MCP_PLUS_PLUS_INTEROP_DESCRIPTOR = {
  ...
  schema_refs: {
    ...
    ...MCP_PLUS_PLUS_INTEROP_DESCRIPTOR_PATHS,
  },
  runtime_handoff: {
    source_surface: 'swissknife',
    target_surface: 'Mcp-Plus-Plus',
    allowed_surfaces: ['agent', 'mcp_server', 'remote_client'],
    required_swissknife_operations: MCP_PLUS_PLUS_INTEROP_OPERATIONS,
...
/**
 * Converts an `MCPPPInterfaceDescriptor` (SwissKnife's internal representation)
 * into the plain payload shape expected by the upstream `Mcp-Plus-Plus`
 * `MCPIDLValidator.validate_descriptor()` entrypoint, i.e. the `payload` shape
 * consumed by
 * Mcp-Plus-Plus/tests-py/fixtures/valid/swissknife_mcp_plus_plus_interop_descriptor.json.
 */
export function toMcpIdlValidatorDescriptor(descriptor: MCPPPInterfaceDescriptor) {
  return {
    ...
    errors: descriptor.errors.map((error) => ({ name: error.name, code: error.code })),
