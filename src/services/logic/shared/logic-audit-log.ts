...
if (!nodeFs) {
  throw new Error('node:fs builtin module is required for AuditLogger file sinks');
}
...
export class AuditLogger {
  ...
  constructor(options: string | null | AuditLoggerOptions = {}) {
    ...
    if (this.logPath) {
      nodeFs.mkdirSync(dirname(this.logPath), { recursive: true });
    }
  }
  ...
  logEvent(
    ...
    if (this.logPath) {
      nodeFs.appendFileSync(this.logPath, `${JSON.stringify(event)}\n`, 'utf8');
    }
    return event;
  }
